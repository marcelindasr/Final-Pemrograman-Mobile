...
fun PopularMovieScreen(
    ...
    if(listMovieState.popularListMovie.isEmpty()) {
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(listMovieState.popularListMovie.size) {index ->
                MovieItem(
                    movie = listMovieState.popularListMovie[index],
                    navHostController = navController
                )
                Spacer(modifier = Modifier.height(16.dp) )

                if (index >= listMovieState.popularListMovie.size - 1 && !listMovieState.isLoading) {
                    onEvent(ListMovieUiEvent.Paginate(Category.POPULAR))
                }
            }
